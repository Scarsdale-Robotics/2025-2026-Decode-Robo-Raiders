...
class AutonRedMotif : NextFTCOpMode() {
    ...
    private val startPathControlP = Pose(blueRedConvertXPos(58.0), 108.0)
    private val endBeforeMotifStartPose =
        Pose(25.0, 100.0, Math.toRadians(270.0)) // End Pose of our robot.

    /**MOTIF GPP/// */ // MotifGPPIntake1
    private val motifGPPIntake1IntakedControlP = Pose(blueRedConvertXPos(73.0), 87.0)
    private val motifGPPIntake1Intaked = Pose(blueRedConvertXPos(20.0), 81.0, Math.toRadians(blueRedConvertAngle(180.0)))

    private val motifGPPIntake1ToShoot = Pose(blueRedConvertXPos(58.0), 94.0)

    //MotifGPPIntake2 (also where it shoots the 2nd time)
    private val motifGPPIntake2Purp1ControlP = Pose(blueRedConvertXPos(95.0), 134.0)
    private val motifGPPIntake2Purp1Intaked = Pose(blueRedConvertXPos(37.0), 56.0)

    private val motifGPPIntake2RestOTPControlP = Pose(blueRedConvertXPos(60.0), 90.0)
    private val motifGPPIntake2RestOfThemPrep = Pose(blueRedConvertXPos(18.536), 69.658, Math.toRadians(blueRedConvertAngle(-70.0)))

    private val motifGPPIntake2Intaked = Pose(blueRedConvertXPos(19.364), 68.7)
    private val motifGPPIntake2Intaked2 = Pose(blueRedConvertXPos(29.0), 67.9)

    /**MOTIF PGP/// */ // MotifPGPIntake1
    private val motifPGPIntake1PrepControlP = Pose(blueRedConvertXPos(43.0), 98.0)
    private val motifPGPIntake1Prep = Pose(blueRedConvertXPos(37.0), 84.0, Math.toRadians(blueRedConvertAngle(180.0)))

    private val motifPGPIntake1IntakedControl = Pose(blueRedConvertXPos(32.0), 112.0)
    private val motifPGPIntake1Intaked = Pose(blueRedConvertXPos(17.45), 93.0, Math.toRadians(blueRedConvertAngle(-70.0)))

    private val motifPGPIntake1Intaked2 = Pose(blueRedConvertXPos(20.9), 90.9)

    private val motifPGPIntake1FinalPos = Pose(blueRedConvertXPos(56.0), 94.0)

    // MotifPGPIntake2
    private val motifPGPIntake2PrepControlP = Pose(blueRedConvertXPos(101.0), 120.0)
    private val motifPGPIntake2Prep = Pose(blueRedConvertXPos(37.0), 60.0, Math.toRadians(blueRedConvertAngle(180.0)))

    private val motifPGPIntake2IntakedControlP = Pose(blueRedConvertXPos(1.5), 58.0)


    /**MOTIF PPG/// */ // MotifPPGIntake1 prep
    private val motifPPGIntake1StartPos = Pose(blueRedConvertXPos(18.2), 92.6, Math.toRadians(blueRedConvertAngle(-65.0)))
    private val motifPPGIntake1MidPos = Pose(blueRedConvertXPos(18.2), 89.5)

    // MotifPPGIntake 1 position
    private val motifPPGIntake1EndPos = Pose(blueRedConvertXPos(59.0), 94.0, Math.toRadians(0.0))

    // MotifPPGIntake2 intake preparation
    private val motifPPGIntake2ControlP1 = Pose(blueRedConvertXPos(94.0), 93.0)
    private val motifPPGIntake2MidPos = Pose(blueRedConvertXPos(37.0), 68.0, Math.toRadians(blueRedConvertAngle(180.0)))

    // MotifPPGIntake2 positions
    private val motifPPGIntake2PushOutPurple = Pose(blueRedConvertXPos(37.0), 57.5)
    private val motifPPGIntake2EndControlP1 = Pose(blueRedConvertXPos(45.0), 88.0)
    private val motifPPGIntake2End = Pose(blueRedConvertXPos(37.0), 42.0, Math.toRadians(270.0))


    /**Motif Park/// */
    private val motifsShooterEnd = Pose(blueRedConvertXPos(75.0), 90.0, Math.toRadians(blueRedConvertAngle(180.0)))

    private val motifBlueParkControlP = Pose(blueRedConvertXPos(54.0), 14.0)
    private val motifBluePark = Pose(blueRedConvertXPos(16.0), 5.0)
    ...
    fun autonomousPathUpdate() {
        when (pathState) {
            AutonPath.StartingPath -> {
                ...
                setPathState(AutonPath.GPPPathIntake1) //for now this
            }
            //GPP PATHS (START)
