...
@Autonomous(name = "WARICK Red Auton", group = "Auton")
@Configurable
class AutonRedWarwick : NextFTCOpMode() {
    ...
    // robot positions
    private val startPose = Pose(144-32.0, 137.0, Math.toRadians(0.0)) // Start Pose of our robot.

    private val intake1stLinePos = Pose(144-11.0, 61.5)
    private val intake1ControlPointPos = Pose(144-73.0, 52.0)

    private val intake2ndLinePos = Pose(144-16.5, 85.5)
    private val intake2ControlPointPos = Pose(144-45.0, 87.0)
    private val intake2FirstBallPos = Pose(144-27.0, intake2ndLinePos.y)
...
//    private val intake3FirstBallPos = Pose(27.0, intake3rdLinePos.y)
    ...
    private val ballReleasePose = Pose(144-13.0, 66.0, Math.toRadians(90.0))

    private val shootingPose = Pose(144-58.0, 80.0, Math.toRadians(0.0)) // The shooter position for everything
    private val endPose = Pose(144-48.0, 80.0) // End Pose of our robot
    ...
    private var robotWarwickGo: PathChain? = null
...
//        )
        ...
        robotWarwickGo = follower!!.pathBuilder()
            .addPath(
                BezierLine(
                    intake1stLinePos,
                    shootingPose
    ...
    fun autonomousPathUpdate() {
        when (pathState) {
            ...
            AutonPath.RobotIntake1 -> if (!follower!!.isBusy) {
                ...
                if (!intakeDone1) {
                    ...
                    if (actionTimer!!.elapsedTimeSeconds >= delayAfterIntake + 0.1) {
                        setPathState(AutonPath.RobotGoWarwick)
                    }
                }
            }
            AutonPath.RobotGoWarwick -> if (!follower!!.isBusy) {
                follower!!.setMaxPower(1.0)
                if (intakeReached4) {
                    stop.schedule()
            ...
            AutonPath.RobotShoot2 -> if (!follower!!.isBusy) {
                ...
                if (pathF2) {
                    follower!!.followPath(robotWarwickGo!!)
                    pathF2 = false
                }
